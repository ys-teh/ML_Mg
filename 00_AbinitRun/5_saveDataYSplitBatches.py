...
##-------------------------------------------------------------------------------
def generatePythonFile(batchNumber,minDataNumber,maxDataNumber,filename):
...
##-------------------------------------------------------------------------------
def load_all_eband_and_entr(ngfft,nfft,nband,minData,maxData,sigma):
    dataSize = maxData - minData + 1
    allEband = np.zeros((dataSize,nfft))
    allEntr = np.zeros((dataSize,nfft))

    for idata in range(dataSize):
        print('data no: ', idata+minData)

        # Read occupation number (occ), eigenvalues (eig) and k point weight (wtk)
        # from NetCDF file
        filename = 'Abinit_output/mg' + str(idata+minData) + '_WFK.nc'
...
def main():
    ...
    minData = %i
    maxData = %i
    Y_EBAND, Y_ENTR = load_all_eband_and_entr(ngfft,nfft,nband,minData,maxData,sigma)
    np.save('mg_Y_EBAND_batch%i.npy',Y_EBAND)
    np.save('mg_Y_ENTR_batch%i.npy',Y_ENTR)
    ...
    np.savetxt('check_total_band_energy_batch%i.txt', total_eband)
    np.savetxt('check_total_entropy_batch%i.txt', total_entr)

if __name__ == '__main__': main()
"""%(minDataNumber,maxDataNumber,batchNumber,batchNumber,batchNumber,batchNumber)
...
#################################################################################
def generateSbatchScript(batchNumber, minDataNumber, maxDataNumber, filename):
    fileContent = """#!/bin/sh
############################################################################
# RUN 5_SAVEDATAY_BATCHXX.PY
############################################################################
...
python3 5_saveDataY_batch%i.py
...
def main():
    startDataNumber = 1
    endDataNumber = 3000
    dataSize = endDataNumber - startDataNumber + 1
    batchSize = 50
    totalBatches = int(math.ceil(float(dataSize)/float(batchSize)))

    # Generate python and sbatch scripts
    for batchNumber in range(1,totalBatches+1):
        minDataNumber = batchSize*(batchNumber-1) + startDataNumber
        maxDataNumber = min(batchSize*batchNumber+startDataNumber-1, endDataNumber)
        filename = '5_saveDataY_batch' + str(batchNumber) + '.py' 
        generatePythonFile(batchNumber,minDataNumber,maxDataNumber,filename)
        filename = 'run_batch' + str(batchNumber) +'.sh'
        generateSbatchScript(batchNumber,minDataNumber,maxDataNumber,filename)
