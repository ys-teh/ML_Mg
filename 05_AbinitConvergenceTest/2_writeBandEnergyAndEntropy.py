...
def load_all_eband_and_entr(ngfft,nfft,nband,sigma):
    allEband = np.zeros((nfft))
    allEntr = np.zeros((nfft))

    # Read occupation number (occ), eigenvalues (eig) and k point weight (wtk)
    # from NetCDF file
    filename = 'Output/mg_WFK.nc'
    ...
    for ikpt in range(nkpt):
        ...
        for iband in range(nband):
            ...
            allEband += (eig[0,ikpt,iband] * occKbd * wt_k * nfft) * psiSquare

            a = -0.5634
            x = (fermie - eig[0,ikpt,iband])/sigma
            sca = (1./math.sqrt(math.pi)) * math.exp(-x*x) * (x*x*(-a*x+1) - 0.5)
            allEntr += (-sca * wt_k * nfft) * psiSquare
...
def main():
    ...
    total_eband = np.sum(Y_EBAND)
    total_entr = -np.sum(Y_ENTR)*sigma
    print('check_total_band_energy_batch.txt', total_eband)
    print('check_total_entropy_batch.txt', total_entr)
